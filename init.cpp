...
#include "config/yeahreum-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "yeahreum (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Yeahreum specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N YAH anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/yeahreum>";
    const std::string URL_WEBSITE = "<https://yeahreum.com>";
...
/** Sanity checks
 *  Ensure that Yeahreum Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Yeahreum Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Yeahreum Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    //lite mode disables all Yeahreum-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Yeahreum-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/yeahreum/pull/1817 and https://github.com/yeahreum/pull/1743
    ...
    // ********************************************************* Step 11c: update block tip in Yeahreum modules
    ...
    // ********************************************************* Step 11d: start yeahreum-ps-<smth> threads
